# task2/udpclient.py
# UDP 可靠传输模拟——客户端 (GBN 发送方)
# 运行环境: Windows Host OS
# 用法: python udpclient.py <serverIP> <serverPort> <filepath>
#
# 功能:
#   - 模拟 TCP 连接建立 (自定义应用层首部)
#   - GBN 滑动窗口协议: 窗口大小=5, 超时=300ms
#   - 每 UDP 报文载荷 ≤ 80 字节
#   - 超时重传, 累计 ACK 处理
#   - RTT 计算 + 服务器系统时间显示
#   - 30 个报文后统计: 丢包率, RTT(max/min/avg/stddev)
#   - 自动生成 run_log.txt
#
# 自定义协议首部 (同 udpsvr.py)
...
WINDOW_SIZE = 5        # 发送窗口大小 (至少 5)
PAYLOAD_SIZE = 80      # 每个 UDP 载荷字节数
TARGET_PACKETS = 30    # 目标发送报文数 (用于统计)
...
def main():
    ...
    for i in range(0, file_size, PAYLOAD_SIZE):
        ...
        packets.append({
            ...
            'data': chunk,
            'start': i,                        # 起始字节偏移
            'end': min(i + PAYLOAD_SIZE, file_size) - 1,  # 结束字节偏移
            'length': len(chunk),
        })
    ...
    acked = [False] * total_packets   # 跟踪每个分组的确认状态
    send_time = {}                     # send_time[seq] = 发送时间戳
    first_send_time = {}              # 首次发送时间 (用于丢包统计)
    rtt_values = []                    # 所有 RTT 样本
    total_sent = 0                     # 总发送次数 (含重传)

    send_base = 0                      # SR 发送窗口基序号
            ...
            try:
                ...
                if send_base <= ack_num < send_base + WINDOW_SIZE:
                    if not acked[ack_num] and ack_num in send_time:
                        rtt = (now - send_time[ack_num]) * 1000  # ms
                    ...
                    # 推进 send_base (跳过已确认的连续前缀)
                    while send_base < total_packets and acked[send_base]:
                        send_base += 1

                # ---- 快重传: 窗口内 ≥3 个包已确认但 send_base 未确认 → 立即重传 send_base ----
    ...
    # ---- 6. 统计信息 ----
    print(f"\n===== 阶段 3: 统计信息 =====")
    actual_packets = total_packets  # 唯一报文数量
    print(f"目标报文数: {actual_packets}")
    print(f"实际发送次数 (含重传): {total_sent}")
    print(f"成功确认数: {sum(acked)}")

    # 丢包率 = (实际发送 - 唯一报文数) / 实际发送
    # 或按实验要求: delivery_ratio = actual_packets / total_sent (百分比)
    if total_sent > 0:
